...
void registerInputBindings(py::module_ &m)
{
    m.def("is_key_down", [](int scancode)
          { return EngineBindings::getInputManager()->isKeyDown(static_cast<SDL_Scancode>(scancode)); });

    m.def("is_key_pressed", [](int scancode)
          { return EngineBindings::getInputManager()->isKeyPressed(static_cast<SDL_Scancode>(scancode)); });

    m.def("is_key_released", [](int scancode)
          { return EngineBindings::getInputManager()->isKeyReleased(static_cast<SDL_Scancode>(scancode)); });
